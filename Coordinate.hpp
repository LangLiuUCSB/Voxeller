...
struct Coordinate
{
  ...
  bool operator==(const Coordinate &other) const noexcept { return x == other.x && y == other.y && z == other.z; }
};

inline std::ostream &operator<<(std::ostream &s, const Coordinate &c) noexcept { return s << '(' << c.x << ',' << c.y << ',' << c.z << ')'; }
size_t manhattan_distance(const Coordinate &c1, const Coordinate &c2) noexcept { return abs(c1.x - c2.x) + abs(c1.y - c2.y) + abs(c1.z - c2.z); }
struct CoordinateHash
{
  std::size_t operator()(const Coordinate &c) const noexcept { return c.x * 73856093 ^ c.y * 19349663 ^ c.z * 83492791; }
};
